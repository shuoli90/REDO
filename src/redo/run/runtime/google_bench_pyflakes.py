...
if __name__ == '__main__':
    ...
    count = 0
    results = []
    save_file_name = f'google_bench_{args.seed}_{args.split}_pyflakes.jsonl' if not args.no_error else f'google_bench_noerror_{args.seed}_{args.split}_pyflakes.jsonl'
    for record in tqdm(indices):
        ...
        if os.path.exists(implementation_path) and os.path.exists(description_path):
            ...
            # pylint_output = run_pylint(implementation_path)
            output_str, error_str = run_pyflakes(implementation)
            # insert pyright
            ...
            if len(sample_inputs) == 0:
                ...
                if len(running_error_types) == 0:
                    ...
                    running_types = []
                    for error_tmp in running_error_types:
                        try:
                            running_type = error_to_index[error_tmp]
                        except KeyError:
                            running_type = 30
