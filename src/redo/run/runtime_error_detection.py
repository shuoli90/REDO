...
if __name__ == "__main__":
    ...
    for idx, pred in tqdm(preds.iterrows(), total=len(preds)):
        ...
        result['instance_id'] = pred['instance_id']
        # result['model_patch'] = new_patch
        result['conclusion'] = conclusion
        # result['model_name_or_path'] = predicted_file + '_new'
        result['response'] = response
        # result['fix_response'] = fix_response
        # result['patch'] = pred['model_patch']
        ...
        if idx % 10 == 0:
             # save results
            with open(f'/home/ubuntu/mnt/agent/amazon-Q/NGDEBirds/NGDEBirdsScienceTransforms/src/birds_transforms/examples/runtime_detection_predicted_{predicted_file}_results_ant_singleshot_new2.jsonl', 'w') as f:
                for result in results:
                    f.write(json.dumps(result) + '\n')

    with open(f'/home/ubuntu/mnt/agent/amazon-Q/NGDEBirds/NGDEBirdsScienceTransforms/src/birds_transforms/examples/runtime_detection_predicted_{predicted_file}_results_newpatch_singleshot_new2.jsonl', 'w') as f:
        for result in results:
            f.write(json.dumps(result) + '\n')