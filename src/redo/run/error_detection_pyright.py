...
# def filter_condition(issue_message):
#     if 'possibly unbound' in issue_message:
#         return False
#     if 'None' in issue_message:
#         return False
#     if 'Unknown' in issue_message:
#         return False
#     if "is obscured by a declaration" in issue_message:
#         return False
#     return True
...
if __name__ == '__main__':
    ...
    for idx, pred in tqdm(preds.iterrows(), total=len(preds)):
        ...
        for file in related_files:
            ...
            # first use ast to check syntax and identation errors
            # try:
            #     tree = ast.parse(modified_script)
            # except SyntaxError:
            #     syntax_instances.append(pred['instance_id'])
            #     continue
        ...
        # for original_script, modified_script in zip(original_scripts, modified_scripts):
        #     original_functions, original_attributes = extract_functions_and_attributes(original_script)
        #     predicted_functions, predicted_attributes = extract_functions_and_attributes(modified_script)
        #     # find new attribute variables and functions
        #     func_diff = []
        #     for predicted_function in predicted_functions:
        #         if predicted_function not in original_functions:
        #             func_diff.append(predicted_function)
        #     attri_diff = []
        #     for predicted_attribute in predicted_attributes:
        #         if predicted_attribute not in original_attributes:
        #             attri_diff.append(predicted_attribute)
            
            
        #     if len(func_diff) == 0:
        #         print("No changes in function signatures found.")
        #     else:
        #         # print("New functions:")
        #         for func_name in func_diff:
        #             # locs = find_function_in_repo(
        #             #     repo_path=repo_dir,
        #             #     function_name=func_name)
        #             # if len(locs) == 0:
        #             warning_instances.append(pred['instance_id'])
        #             continue
        ...
        #         for attri_name in attri_diff:
        #             # print(attri_diff)
        #             # classes = find_classes_with_attribute(
        #             #     repo_path=repo_dir,
    ...
    results = {
        ...
        'original_issues': original_issues_list,
        'modified_issues': modified_issues_list
    }
    # write results to a json file
    if args.golden:
        with open(f'/home/ubuntu/mnt/agent/amazon-Q/NGDEBirds/NGDEBirdsScienceTransforms/src/birds_transforms/examples/golden_results_pyright.json', 'w') as f:
            json.dump(results, f)
    else:
        with open(f'/home/ubuntu/mnt/agent/amazon-Q/NGDEBirds/NGDEBirdsScienceTransforms/src/birds_transforms/examples/predicted_{predicted_file}_results_pyright.json', 'w') as f:
            json.dump(results, f)
