...
TEMPLATE = """
Given the implemented script, please check if the implementation contains runtime errors. \
Please assume that the inputs are always valid; and only reflect the most common case.
...
def identify_pyright_error(error_message: str) -> dict:
    error_types = {
        'Type Error': [
            ...
            'Argument of type', 'cannot be applied', 'TypeVar', 'types', 'Object of type'
        ],
        'Syntax Error': [
            'invalid syntax', 'unexpected token', 'SyntaxError'
        ],
        'Import Error': [
            'cannot import', 'No module named', 'ImportError', 'cannot be resolved'
        ],
        'Name Error': [
            'is not defined', 'unresolved reference', 'NameError'
        ],
        'Attribute Error': [
            'Cannot access member', 'has no attribute', 'AttributeError', 'Cannot access attribute',
        ],
        'Index Error': [
...
if __name__ == '__main__':
    ...
    count = 0
    results = []
    save_file_name = f'google_bench_nocontext_{args.seed}_{args.split}_SONNET.jsonl' if not args.no_error else f'google_bench_nocontext_noerror_{args.seed}_{args.split}_SONNET.jsonl'
    ...
    for record in tqdm(indices):
        ...
        if os.path.exists(implementation_path) and os.path.exists(description_path):
            ...
            count += 1
            print('Progress:', count / len(indices))
